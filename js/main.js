...
(function ($) {
	...
	--------------------*/
	/*------------------
	Image Popup with Download Button
--------------------*/
	$('.img-popup').magnificPopup({
		...
		removalDelay: 500,
		gallery: {
			enabled: true // Enable gallery mode if there are multiple images
		},
		callbacks: {
			markupParse: function (template, values, item) {
				// Adding the download button dynamically
				var downloadUrl = item.src; // Get the image URL
				var downloadButtonHtml = '<a href="' + downloadUrl + '" class="download-btn" download="' + downloadUrl.split('/').pop() + '" style="display:block;text-align:center;margin-top:10px;color:#fff;background:#333;padding:10px 20px;border-radius:5px;">Download Image</a>';

				// Append download button after the image
				template.find('.mfp-bottom-bar').append(downloadButtonHtml);
			}
		}
	});
